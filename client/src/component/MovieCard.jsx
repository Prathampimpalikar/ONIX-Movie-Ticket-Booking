...
const MovieCard = ({ movie }) => {
    ...
    return (
        <div className='flex flex-col justify-between p-3 bg-gray-800 rounded-2xl hover:-translate-y-1 transition duration-300 w-64'>
            <img onClick={() => { navigate(`/movie/${movie._id || movie.id}`); window.scrollTo(0, 0) }}
            ...
            <div className='flex items-center justify-between mt-2'>
                <button onClick={() => { navigate(`/movie/${movie._id || movie.id}`); window.scrollTo(0, 0) }} className='px-4 py-2 text-xs bg-primary hover:bg-primary-dull transition rounded-full font-medium'>Buy ticket</button>
                <p className='flex items-center gap-1'>
                    <Star className="w-5 h-4 text-primary fill-primary" />
